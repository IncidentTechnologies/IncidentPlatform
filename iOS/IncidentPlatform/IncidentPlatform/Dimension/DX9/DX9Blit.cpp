...
DX9Blit::DX9Blit( point origin, float height, float width, BLIT_TYPE type ) :
blit(origin, height, width, type),
m_pd39Buf(NULL),
m_ppTextures(NULL),
m_pVertexShader(NULL),
m_pVertexConstants(NULL),
...
DX9Blit::DX9Blit( FlatObject *obj ) :
blit(static_cast<blit*>(obj)->GetOrigin(), 
     static_cast<blit*>(obj)->GetHeight(),
     static_cast<blit*>(obj)->GetWidth(),
     static_cast<blit*>(obj)->GetBlitType(),
     static_cast<blit*>(obj)->GetScreenWidth(),
     static_cast<blit*>(obj)->GetScreenHeight()),
     m_pd39Buf(NULL),
     m_ppTextures(NULL),
     m_pVertexShader(NULL),
     m_pVertexConstants(NULL),
...
RESULT DX9Blit::Initialize( DimensionImp *pDimImp )
{
    HRESULT hr = S_OK;
    RESULT r = R_OK;

    LPDIRECT3DDEVICE9 pd3device9 = reinterpret_cast<DimensionImpDX9 *>(pDimImp)->GetDirectX9Device();
    CHM(pd3device9, "DX9Blit: Could not get DirectX9 Device!\n");
    ...
    CHRM(hr, "DX9Blit::failed to create vertex shader");
    ...
    CHRM(hr, "DX9Blit::failed to create pixel shader");
...
RESULT DX9Blit::Render( DimensionImp* pDimImp )
{
    RESULT r = R_OK;
...
RESULT DX9Blit::Render( LPDIRECT3DDEVICE9 pd3device9 )
{
    RESULT r = R_OK;
    HRESULT hr = S_OK;
