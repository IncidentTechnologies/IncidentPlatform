                                          ...
                                          )
{
    ...
    // reset camera 
    if(pDX9ObjCaller != NULL && 
       pDX9ObjCaller->GetMaterialShadeStyle() == SHADE_PHONG_REFLECT && 
       pReflectionCamera != NULL)
    {
        pReflectionCamera->Render(m_pd3device9);
    }
                                          ...
                                          )
{
    ...
    DX9Camera *pLocalReflectionCamera = NULL;
    ...
    {        
        ...
        if(pDX9ObjCaller != NULL)
        {
            ...
            {               
                ...
                //pRefractionCamera->Render(m_pd3device9);
                pLocalReflectionCamera = new DX9Camera(CameraReflectionPoint, pRefractionCamera->GetLookAtPosition(), 1.0f * pRefractionCamera->GetUpDirection());
                pLocalReflectionCamera->Render(m_pd3device9);
                pReflectionCamera = pLocalReflectionCamera;                 
                
                ProjMatrix = pLocalReflectionCamera->GetProjectionMatrix();
                ViewMatrix = pLocalReflectionCamera->GetViewMatrix();
            }
            else if(pDX9ObjCaller->GetMaterialShadeStyle() == SHADE_PHONG_REFLECT && pReflectionCamera != NULL)
            {
                ...
                //pReflectionCamera->Render(m_pd3device9);
                pLocalReflectionCamera = new DX9Camera(CameraReflectionPoint, pReflectionCamera->GetLookAtPosition(), 1.0f * pReflectionCamera->GetUpDirection());
                pLocalReflectionCamera->Render(m_pd3device9);
                pReflectionCamera = pLocalReflectionCamera;                 
        ...
        m_pd3device9->BeginScene();
        {
            ...
            // the object as the caller.  So when the render function is reached for this object it will be skipped
            pObjectStore->GetDimensionObjectDataBase()->Iterate((kdtree::FunctionPtrArgDepthObjCams)RenderDimObject, m_pDimImpDX9, depth + 1, pDimObj, NULL, pLocalReflectionCamera);
        }
        m_pd3device9->EndScene(); 
