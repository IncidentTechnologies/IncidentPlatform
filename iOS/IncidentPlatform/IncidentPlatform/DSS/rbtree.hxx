...
public:
    ...
    RBTreeNode<T>*remove(RBTreeNode<T>*node)
    {
        ...
        if(node->m_left->m_data == NULL)
            RESULT r = RBRemove(node);  // Now we have a node with at most one child
...
public:
    ...
    RESULT Insert(T data)
    {
        ...
        if(m_root == NULL)
            m_root = new RBTreeNode<T>(data, BLACK_NODE, NULL);                 // If this is case 1
        else
            r = Insert(data, m_root);       // root node has NULL parent / grandparent
